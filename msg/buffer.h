...
int buf_assign (struct buffer *dest, const struct buffer *src);
...
static inline int has_digit (const unsigned char* src)
{
  unsigned char c;
  while ((c = *src++))
    {
 ...
 */
int buf_parse (struct buffer *buf, const int delim, char *line, const int size);
 ...
 */
struct buffer buf_sub (struct buffer *buf, int size, int prepend);
...
static inline uint8_t *
buf_write_alloc_prepend (struct buffer *buf, int size, int prepend)
{
...
buf_read_u32 (struct buffer *buf, int *good)
{
  uint32_t ret;
...
int buf_string_match_head_str (const struct buffer *src, const char *match);
int buf_string_compare_advance (struct buffer *src, const char *match);
...
int char_class (const unsigned char c, const unsigned int flags);
int string_class (const char *str, const unsigned int inclusive, const unsigned int exclusive);
int string_mod (char *str, const unsigned int inclusive, const unsigned int exclusive, const char replace);
